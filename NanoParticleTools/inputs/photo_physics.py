...
def gaussian(x, c, sigma) -> float:
    """
    Mimics the functionality of gauss() in Igor Pro

    :param x:
    :param c:
    :param sigma:
    :return: the value of a normalized gaussian
    """
...
def get_MD_line_strength_from_icc(initial_intermediate_coupling_coefficients: np.ndarray,
                                  final_intermediate_coupling_coefficients: np.ndarray,
                                  ji: float,
                                  jf: float,
                                  s: np.ndarray,
                                  l: np.ndarray) -> float:
    ...
    :return:
    """
    if len(initial_intermediate_coupling_coefficients) != len(final_intermediate_coupling_coefficients):
        raise ValueError(
            "Number of rows in initial_intermediate_coupling_coefficient and final_intermediate_coupling_coefficients do not match")
    elif len(initial_intermediate_coupling_coefficients) != len(s):
        raise ValueError("Number of rows in initial_intermediate_coupling_coefficient and s do not match")
    elif len(initial_intermediate_coupling_coefficients) != len(l):
        raise ValueError("Number of rows in initial_intermediate_coupling_coefficient and l do not match")

    m_sum = 0
    for i, (eigenvector_i, eigenvector_f) in enumerate(
            zip(initial_intermediate_coupling_coefficients, final_intermediate_coupling_coefficients)):
        ...
        m_sum += eigenvector_i * eigenvector_f * magnetic_dipole_operation(si, li, ji, sf, lf, jf)
    return m_sum ** 2


def magnetic_dipole_operation(si: float,
                              li: float,
                              ji: float,
                              sf: float,
                              lf: float,
                              jf: float) -> float:
    """
    returns the  expectation value |<i|M|j>|^2, where M is the magnetic dipole operator, M = |L + 2S|, in gaussian cgs units (erg/G)
...
def get_absorption_cross_section_from_MD_line_strength(line_strength: float,
                                                       energy_gap: float,
                                                       j_init: float,
                                                       n_refract: float) -> float:
    """
    ...
    oscillator_strength = get_oscillator_strength_from_MD_line_strength(line_strength, energy_gap, j_init, n_refract)
...
def get_rate_from_MD_line_strength(line_strength: float,
                                   energy_gap: float,
                                   j_init: float,
                                   n_refract: float) -> float:
    """
    ...
    :return:
    """
    return 64 * np.pi ** 4 * abs(energy_gap) ** 3 * n_refract ** 3 / (3 * h_CGS * (2 * j_init + 1)) * line_strength


def gaussian_overlap_integral(energy_gap: float,
                              fwhm: float) -> float:
    """

    port of PP_GaussianOverlapIntegral
    :param energy_gap: The distance between the donor emission and acceptor absorption peak centers, assuming gaussian lineshapes
    :param fwhm:
    :return: The overlap integral of two normalized gaussian peaks with fwhm fwhm and peak centers separated by Egap
    """
    return fwhm ** (-1) * np.sqrt(2 * np.log(2) / np.pi) * 2 ** (-2 * (energy_gap / fwhm) ** 2)
...
def phonon_assisted_energy_transfer_constant(donor_line_strength: float,
    ...
    :return:
    """
    et_constant = energy_transfer_constant(donor_line_strength, acceptor_line_strength, overlap_integral, n_refract,
                                           j_di, j_ai)
...
def energy_transfer_constant(donor_line_strength: float,
    ...
    return 8 * np.pi ** 2 * e_CGS ** 4 * overlap_integral / (
        3 * h_CGS ** 2 * c_CGS * donor_degeneracy * acceptor_degeneracy) * correction_refractive_index ** 2 * donor_line_strength * acceptor_line_strength
