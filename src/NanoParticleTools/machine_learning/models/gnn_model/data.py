...
class LayerNodeFeatureProcessor(FeatureProcessor):
    ...
    def graph_from_inputs(self, dopant_concentration: dict,
        ...
        data['dopant_concs'] = torch.tensor(
            [i[1] for i in dopant_specifications],
            dtype=torch.float32,
            requires_grad=self.input_grad).reshape(-1,
                                                   self.n_possible_elements)
        data['dopant_constraint_indices'] = torch.arange(0, len(data['dopant_concs']))
        data['num_nodes'] = len(data['dopant_concs'])

        edge_index = []
        for i, _ in enumerate(data['dopant_concs']):
            for j, _ in enumerate(data['dopant_concs']):
...
class GraphInteractionFeatureProcessor(FeatureProcessor):
    ...
    def graph_from_inputs(self, dopant_concentration: dict,
        ...
        data['dopant_concs'] = torch.tensor(dopant_concs,
                                            dtype=torch.float32,
                                            requires_grad=self.input_grad)
        data['dopant_types'] = torch.tensor(dopant_types, dtype=torch.long)
        data['dopant_constraint_indices'] = torch.tensor(
