...
# from .._ray import *
from NanoParticleTools.machine_learning.models.transformer_model.model import SpectrumAttentionModel
from NanoParticleTools.machine_learning.models.transformer_model.data import (
    TransformerFeatureProcessor)
from NanoParticleTools.machine_learning.data.processors import LabelProcessor
...
def train_spectrum_model(config: dict,
    ...
    :param config: a config dictionary for the model
        ex - {'n_input_nodes': 20, 'n_output_nodes'; 200, 'n_hidden_layers': 2,
              'n_hidden_1': 128, 'n_hidden_2': 128, 'learning_rate': 1e-3}
    :param num_epochs: max number of epochs to train the model.
        Early stopping may result in less training epochs
    :param num_gpus: number of gpus to use for the training.
        May be a fractional number if training multiple models concurrently on one gpu
    :param wandb_project: wandb_project name
    :param wandb_save_dir: Directory to save wandb files to
    :param tune: whether or not this is a tune experiment.
        (To make sure that TuneReportCallback is added)
    """
    # Setup the data
    feature_processor = TransformerFeatureProcessor(fields=[
        'formula_by_constraint', 'dopant_concentration', 'input.constraints'
    ])
    label_processor = LabelProcessor(
        fields=['output.spectrum_x', 'output.spectrum_y'],
    ...
    if os.path.exists('npmc_data.json'):
        data_module = NPMCDataModule(feature_processor=feature_processor,
                                     label_processor=label_processor,
                                     data_dir='npmc_data.json',
                                     batch_size=16)
    else:
        data_store = MongoStore.from_launchpad_file(LAUNCHPAD_LOC,
                                                    'avg_npmc_20220708')
        data_module = NPMCDataModule(feature_processor=feature_processor,
                                     label_processor=label_processor,
    ...
    columns = [
        'name', 'nanoparticle', 'spectrum', 'zoomed_spectrum', 'loss',
        'npmc_qy', 'pred_qy'
    ]
    data = []
    ...
    for i in rng.choice(range(len(data_module.npmc_test)), 10, replace=False):
        ...
        plt.plot(data_module.label_processor.x,
                 np.power(10, y.numpy()) - 1,
                 label='NPMC',
                 alpha=1)
        plt.plot(data_module.label_processor.x,
                 np.power(10,
                          y_hat.detach().numpy()) - 1,
                 label='NN',
                 alpha=0.5)
        ...
        fig.canvas.draw()
        full_fig_data = np.frombuffer(fig.canvas.tostring_rgb(),
                                      dtype=np.uint8)
        full_fig_data = full_fig_data.reshape(
            fig.canvas.get_width_height()[::-1] + (3, ))

        plt.ylim(0, 1e4)
        fig.canvas.draw()
        fig_data = np.frombuffer(fig.canvas.tostring_rgb(), dtype=np.uint8)
        fig_data = fig_data.reshape(fig.canvas.get_width_height()[::-1] +
                                    (3, ))

        constraints, dopant_specifications = get_np_template_from_feature(
            types, volumes, compositions, data_module.feature_processor)
        nanoparticle = plot_nanoparticle(constraints,
                                         dopant_specifications,
                                         as_np_array=True)

        npmc_qy = torch.sum(y[int(y.size()[-1] / 2):], dim=-1) / torch.sum(
            y[:int(y.size()[-1] / 2)], dim=-1)
        pred_qy = torch.sum(y_hat[int(y_hat.size()[-1] / 2):],
                            dim=-1) / torch.sum(
                                y_hat[:int(y_hat.size()[-1] / 2)], dim=-1)
...
def tune_npmc_asha(config: dict,
    ...
    reporter = TrialTerminationReporter(
        parameter_columns=[
            "embedding_dimension", "n_heads", "nn_layers", "learning_rate",
            "l2_regularization_weight", "dropout_probability"
        ],
