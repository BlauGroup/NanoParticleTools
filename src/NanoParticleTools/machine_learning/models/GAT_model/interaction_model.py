...
class GATSpectrumModel(SpectrumModelBase):
    def __init__(self,
        ...
        self.conv1 = InteractionConv(input_dim=embed_dim, output_dim=embed_dim, cat_embedding=True)
        self.conv2 = InteractionConv(input_dim=2*embed_dim, output_dim=embed_dim, cat_embedding=True)
        
        # Build the mlp layers
        if readout_operation.lower() == 'set2set':
            mlp_sizes = [2*n_dopants*2*embed_dim]+mlp_layers+[n_output_nodes]
        else:
            mlp_sizes = [n_dopants*2*embed_dim]+mlp_layers+[n_output_nodes]
        ...
        if readout_operation.lower() == 'attn':
            ...
            gate_nn = nn.Sequential(*[nn.Linear(n_dopants*2*embed_dim, 1), nn.Softmax(-2)])
            out_nn = nn.Linear(n_dopants*2*embed_dim, n_dopants*2*embed_dim)
            ...
            readout = pyg_nn.aggr.Set2Set(n_dopants*2*embed_dim, processing_steps=7)
    ...
    def forward(self, x, edge_index, edge_attr, batch, **kwargs):
        embedding = self.embeder(torch.arange(0, self.n_dopants, device=x.device).expand(x.shape))
