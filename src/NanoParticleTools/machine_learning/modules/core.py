    ...
    def forward(self, input: torch.Tensor):
        if input.size(0) > 1:
            if self.training and self.track_running_stats:
                ...
                with torch.no_grad():
                    self.running_mean = (1 - self.momentum) * self.running_mean + self.momentum * input.mean(0)
                    # self.running_var = (1 - self.momentum) * self.running_var + self.momentum * input.var(0, unbiased=False)
