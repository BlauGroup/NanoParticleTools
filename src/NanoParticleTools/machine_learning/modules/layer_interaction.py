...
class InteractionConv(MessagePassing):
    def __init__(self, 
        ...
        message_mlp_layers = [nn.Linear(message_in_dim, 128),
                              nn.ReLU(),
                              nn.Dropout(0.25),
        ...
        alpha_mlp_layers = [nn.Linear(self.interaction_block.output_dim, 128),
                            nn.ReLU(),
                            nn.Dropout(0.25),
                            nn.Linear(128, 1)]
    ...
    def forward(self,
        ...
        comps = comps[..., None, None].expand(*comps.shape,
                                              comps.size(-1),
    ...
    def message(self, x_j: Tensor, x_i: Tensor, edge_attr: OptTensor,
        ...
        weighted_messages = torch.einsum('ijkl, ijk -> ijkl',
                                         mlp_messages,
                                         message_weights)
...
def integrated_gaussian_interaction(ri0: torch.Tensor, 
                                    rif: torch.Tensor, 
                                    rj0: torch.Tensor, 
                                    rjf: torch.Tensor, 
    ...
    Args:
        ...
        exp_fn (Callable, optional): Exponential function or function that 
            takes its place. Defaults to torch.exp, but another function may
            be specified (untested).
        erf_fn (Callable, optional): Error function or a function that behaves
            similarly. Defaults to torch.erf, but torch.tanh should be 
            substitutable, since they behave similarly.
...
# @torch.jit.script
# Not sure why, but the jit script is slower than the python function here
def integrated_gaussian_interaction_tanh(ri0: torch.Tensor, 
                                    rif: torch.Tensor, 
                                    rj0: torch.Tensor, 
                                    rjf: torch.Tensor, 
                                    sigma: torch.Tensor) -> torch.Tensor:                 
    ...
    Args:
        ...
        rjf (torch.Tensor): Outer radius of the second shell
        sigma (torch.Tensor): Values for standard deviation in the gaussian
        exp_fn (Callable, optional): Exponential function or function that takes its place. 
            Defaults to torch.exp, but another function may be specified (untested).
        erf_fn (Callable, optional): Error function or a function that behaves similarly. 
            Defaults to torch.erf, but torch.tanh should be substitutable, since they behave similarly.
    ...
    term1 = sqrt2_pi3_2_sigma6 * (2 * sigma * (exp1-exp2) - sqrt2pi*rj0*(tanh1-tanh2))
    term2 = 2/3 * pi2 * torch.pow(sigma, 4) * \
        (-exp1*sqrt2_pi*sigma*(ri0**2+ri0*rj0+rj0**2+2*sigma**2) + \
         exp2*sqrt2_pi*sigma*(rif**2+rif*rj0+rj0**2+2*sigma**2) - \
    ...
    tanh1 = torch.tanh((ri0+rj0)/(sqrt2_sigma))
    tanh2 = torch.tanh((rif+rj0)/(sqrt2_sigma))
    term3 = sqrt2_pi3_2_sigma6 * (2 * sigma * (exp1-exp2) + sqrt2pi*rj0*(tanh1-tanh2))
    term4 = 2/3 * pi2 * torch.pow(sigma, 4) * \
        (-exp1*sqrt2_pi*sigma*(ri0**2-ri0*rj0+rj0**2+2*sigma**2) + \
         exp2*sqrt2_pi*sigma*(rif**2-rif*rj0+rj0**2+2*sigma**2) - \
    ...
    term5 = sqrt2_pi3_2_sigma6 * (2 * sigma * (exp1-exp2) - sqrt2pi*rjf*(tanh1-tanh2))
    term6 = 2/3 * pi2 * torch.pow(sigma, 4) * \
        (-exp1*sqrt2_pi*sigma*(ri0**2+ri0*rjf+rjf**2+2*sigma**2) + \
         exp2*sqrt2_pi*sigma*(rif**2+rif*rjf+rjf**2+2*sigma**2) - \
    ...
    tanh1 = torch.tanh((ri0+rjf)/(sqrt2_sigma))
    tanh2 = torch.tanh((rif+rjf)/(sqrt2_sigma))
    term7 = sqrt2_pi3_2_sigma6 * (2 * sigma * (exp1-exp2) + sqrt2pi*rjf*(tanh1-tanh2))
    term8 = 2/3 * pi2 * torch.pow(sigma, 4) * \
        (-exp1*sqrt2_pi*sigma*(ri0**2-ri0*rjf+rjf**2+2*sigma**2) + \
         exp2*sqrt2_pi*sigma*(rif**2-rif*rjf+rjf**2+2*sigma**2) - \
