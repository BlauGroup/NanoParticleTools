...
class ShellInteractionBlock(torch.nn.Module):
    r"""
    This interaction layer will approximate the integral of point-wise interactions
    between two layers
    ...
    def forward(self, sh_coeffs1, sh_coeffs2):
        out = numeric_shell_interaction(sh_coeffs1, sh_coeffs2, self.npoints,
                                        self.sigma, self.spherical_tensor,
                                        self.interaction_type)
        return out


@torch.jit.script
def gaussian_interaction(s: torch.Tensor, sigma: int = 10) -> torch.Tensor:
    return 1 / (sigma * torch.sqrt(torch.pi * torch.tensor(2))) * torch.exp(
        -0.5 * (s / sigma)**2)
...
def numeric_shell_interaction(sh_1,
                              sh_2,
                              npoints,
                              sigma,
                              spherical_tensor,
    ...
    points1 = torch.einsum('...ij, ...i -> ...ij', sphere_points,
                           r1).unsqueeze(-2).expand(*r1.shape,
                                                    sphere_points.size(0), 3)
    points2 = torch.einsum('...ij, ...i -> ...ij', sphere_points,
                           r2).unsqueeze(-3).expand(*r2.shape,
                                                    sphere_points.size(0), 3)
    ...
    if interaction == 'gaussian':
        ...
        raise ValueError(
            f'Requested interaction type {interaction} is unknown')

    # Perform the numerical integration (analogous to a double riemann sum, but in 2d)
    interaction = (dI * dS * dS).sum((-1, -2))
    return interaction


def fibonacci_sphere(samples=1000):
    """
    Adapted from this stack overflow question and answer:
