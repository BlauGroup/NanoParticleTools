...
class FiLMLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if intermediate_dim is not None:
            self.film_gen = NonLinearFiLMGenerator(conditioning_dim,
                                                   feature_dim,
                                                   intermediate_dim)
        else:
...
class FiLMGenerator(torch.nn.Module):
    ...
    def __init__(self, conditioning_dim: int, feature_dim: int):
        super().__init__()
        self.gamma_mlp = nn.Sequential(nn.Linear(conditioning_dim,
                                                 feature_dim))
...
class NonLinearFiLMGenerator(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.gamma_mlp = self.get_mlp(conditioning_dim, intermediate_dim,
                                      feature_dim)
        self.beta_mlp = self.get_mlp(conditioning_dim, intermediate_dim,
                                     feature_dim)

    @staticmethod
    def get_mlp(in_dim: int, dims: List[int], out_dim: int) -> nn.Module:
        modules = []
