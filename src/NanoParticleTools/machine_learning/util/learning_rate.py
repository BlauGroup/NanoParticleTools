from torch.optim.lr_scheduler import LinearLR, ExponentialLR, SequentialLR, ReduceLROnPlateau, EPOCH_DEPRECATION_WARNING
import warnings
...
def get_reduce_with_warmup(optimizer):
    return ReduceLROnPlateauWithWarmup(warmup_epochs=10, patience=100, optimizer=optimizer, factor=0.8)
...
class ReduceLROnPlateauWithWarmup(ReduceLROnPlateau):
    ...
    def _initial_step(self):
        ...
        self.initial_lr = [float(param_group['lr']) for i, param_group in enumerate(self.optimizer.param_groups)]
    ...
    def _increase_lr(self, epoch):
        for i, (param_group, initial_lr) in enumerate(zip(self.optimizer.param_groups, self.initial_lr)):
