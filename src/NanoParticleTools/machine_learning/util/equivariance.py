...
def get_doc(constraints, dopant_specifications):
    return {
        'input': {
...
def create_additive_vectors(n_vecs: int = 4,
                            dim: int = 8):
...
def check_vec_scalar_transform(module: Callable,
    ...
    v1 = _module(sum_vecs)
    v2 = _module(vecs)
    assert vectors_match(v1, v2, print_match=print_match, atol=atol), ('The vector sums do not '
...
def check_vec_vec_transform(module, print_match=True, **kwargs):
    ...
    # Follow the paths
    i, j = torch.cartesian_prod(torch.arange(vecs1.size(0)), torch.arange(vecs2.size(0))).T
...
def check_subdivision_equivariance(model: torch.nn.Module,
    ...
    data = Data(
        **feature_processor.process_doc(get_doc(constraints, dopant_specs)))
